...
public class CharaktereAnsehenView {
    ...
    private void loadView() {
        ...
        CharakterLoescher loeschen = new CharakterLoescher();
        grid.add(loeschen, 1, 0);
        ...
        for (int i = 1; i <= alle.size(); i++) {
            CharakterLabel cLabel = new CharakterLabel(alle.get(i - 1));
    ...
    private class CharakterLoescher extends Button {
        private Charakter charakter;

        public CharakterLoescher() {
            super("Löschen");
            setPadding(new Insets(10, 10, 10,20));
            setOnMouseClicked(e -> {
                if (charakter != null) {
                    controller.deleteCharakter(charakter);
                    app.changeToCharaktereAnsehenView();
                }
            });
        }
